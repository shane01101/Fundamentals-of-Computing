# Shane Honanie
# http://www.codeskulptor.org/#user44_SZJOmJ94ce_3.py
...
# sound assets purchased from sounddogs.com, please do not reribute
...
def draw(canvas):
    ...
    #Restart game if lives become 0    
...
def group_collide(set_group, other_obj):
    flag = False
    for s in list(set_group):
        if s.collide(other_obj):
            set_group.remove(s)
            flag = True
           
    return flag

def group_group_collide(set_group, set_other_group):
    count = 0
    for s in list(set_group):
        if group_collide(set_other_group, s):
            set_group.remove(s)
            count += 1
